<template>
    <Layout>
        <div class="max-w-4xl mx-auto">
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Article</h1>
            </div>
            ...
            <div v-if="!isLoading" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <!-- Form -->
                <form @submit.prevent="updateArticle" class="divide-y divide-gray-100">
                    <!-- Basic Info Section -->
...
const fetchArticle = async () => {
    try {
        ...
        // Set content for TiptapEditor
        content.value = article.content;
        
        // Set tags
        form.value.tags = article.tags.map(tag => tag.id);
        
        // Set featured image if exists
        if (article.featured_image_url) {
            previewImage.value = article.featured_image_url;
        }
        ...
        errors.value = { general: ["Failed to load article. Please try again."] };
    } finally {
...
const removeFeaturedImage = () => {
    ...
    if (form.value.featured_image instanceof File && previewImage.value) {
...
const updateArticle = async () => {
    ...
    try {
        ...
        if (!previewImage.value && !shouldRemoveImage.value) {
        ...
        // Only append featured_image if it's a File object (new upload)
        if (form.value.featured_image instanceof File) {
            formData.append("featured_image", form.value.featured_image);
        }

        // Send form data
        const response = await axios.post(`/api/articles/${articleId}`, formData, {
            headers: {
        ...
        if (error.response && error.response.data.errors) {
            ...
            errors.value = { general: ["An error occurred while updating the article. Please try again."] };
        }
    } finally {
        loading.value = false;
    }
